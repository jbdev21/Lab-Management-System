        ...
        <table class="table">
            ...
            <tbody>
                <tr v-for="recent_sales_order in selectedProduct.recent_sales_order" :key="recent_sales_order.id">
                    <td>{{ recent_sales_order.created_at }}</td>
                    <td>
                        <a target="_blank" :href="`/dashboard/delivery-receipt/${ recent_sales_order.delivery_receipt_id }`">
                            {{ recent_sales_order.delivery_receipt_number }}
                        </a>
                    </td>
                    <td>{{ toPeso(recent_sales_order.unit_price) }}</td>
                </tr>
            </tbody>
        </table>
    </div>  
...
<script>
 export default {
    ...
    methods:{
        ...
        toPeso(number){
            return number.toLocaleString('en-PH', {currency: 'PHP', style: 'currency'});
        },
